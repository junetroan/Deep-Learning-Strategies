...
#SELECT NUMBER OF ITERATIONS FOR SIMULATION
iters = 2
...
@time begin
    for i in 1:iters
        ...
        augmented_u0 = vcat(u0[1], randn(rng3, Float32, 1))
        ...
        CSV.write("Simulations/Results/sim-LV-ANODE-SS/Loss Data/Losses $i.csv", losses_df, writeheader = false)
        ...
        function plot_results(t, real, pred)
            plot(t, pred[1,:], label = "Training Prediction", title="Iteration $i of Randomised ANODE-SS Model", xlabel = "Time", ylabel = "Population")
            scatter!(t, real[1,:], label = "Training Data")
            plot!(legend=:topright)
            savefig("Simulations/Results/sim-LV-ANODE-SS/Plots/Simulation $i.png")
        end
