...
@time begin
    for i in 1:iters
        ...
        CSV.write("Simulations/NODE-MS/Loss Data/Losses $i.csv", losses_df, writeheader = false)
        ...
        function plot_results(real, pred)
            plot(t, pred[1,:], label = "Training Prediction", title="Iteration $i of Randomised NODE-MS Model", xlabel="Time", ylabel="Population")
            scatter!(t, real[1,:], label = "Training Data")
            plot!(legend=:topright)
            savefig("Simulations/NODE-MS/Plots/Simulation $i.png")
        end
