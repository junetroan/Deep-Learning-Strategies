...
t_test = Float32.(collect(Int(round(split_ratio*size(low_price, 1))):size(open_price, 1)))
...
@time begin
    for i in 1:iters
        ...
        ls, ps = multiple_shoot_mod(params, X_train, tsteps, prob_node, loss_function, continuity_loss, AutoTsit5(Rosenbrock23(autodiff = false)), group_size; continuity_term)
