...
function loss_single_shooting(p)
    ...
    loss, pred = multiple_shoot_mod(p, x, tsteps, prob_node, loss_function, continuity_loss, AutoTsit5(Rosenbrock23(autodiff = false)), groupsize; continuity_term)
    #l = loss_function(x, pred[1,:])
    l = loss
    return l, pred
end

ls, ps = loss_single_shooting(params)
...
loss_single_shooting(params)[1]

losses = Float32[]

callback = function (p, l, _)
    push!(losses, loss_single_shooting(p)[1])
